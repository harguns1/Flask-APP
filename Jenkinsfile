pipeline {
  ...
  stages {
    stage('Deploy to Kubernetes') {
      steps {
        sshagent(['ec2-key']) {
          sh 'scp -o StrictHostKeyChecking=no deployment ubuntu@3.83.223.29:/tmp/deployment.yaml'
          sh 'scp -o StrictHostKeyChecking=no service ubuntu@3.83.223.29:/tmp/service.yaml'
